...
<script type="text/javascript">
	...
	$(function() {
				...
				conn.getRoster({
			       success : function(roster) {
			      		...
			      		for (var i = 0; i < roster.length; i++) {
							...
							str +="<tr><td>"+(i+1)+"</td><td><input type='text' name='toname' value='"+roster[i].name+"' readonly='readonly'/><td></tr>";
						}
		...
		dataGrid = $('#dataGrid').datagrid({
			url : '${pageContext.request.contextPath}/userController/fwdataGrid',
			fit : true,
			fitColumns : true,
			border : false,
			pagination : true,
			...
			columns : [ [ {
				...
				formatter : function(value, row, index) {
					...
					str += '&nbsp;';
					str += $.formatString('<img onclick="showFriend(\'{0}\');" src="{1}" title="显示好友信息"/>', row.id, '${pageContext.request.contextPath}/style/images/extjs_icons/tux.png');
					return str;
				}
			} ] ],
			toolbar : '#toolbar',
	...
	function showFriend(id){
		$.post('${pageContext.request.contextPath}/userController/findUser', {
			id : id
		}, function(result) {
			if (result.success) {
				//登录
				var username = result.obj.name;
				var pass = result.obj.pwd;
				conn.open({
					user : username,
					pwd : pass,
					appKey : 'stattimsnw#im'
				});
			}
			parent.$.messager.progress('close');
		}, 'JSON');
...
<body>
		...
		<div data-options="region:'north',title:'查询条件',border:false" style="height: 135px; overflow: hidden;">
			<form id="searchForm">
				<table class="table table-hover table-condensed" style="display: none;">
					<tr>
						<th style="width: 100px;text-align: right;">登录名</th>
						<td><input name="name" placeholder="可以模糊查询姓名" class="span2" /></td>
					</tr>
					<tr>
						<th style="width: 100px;text-align: right;">创建时间</th>
					...
					<tr>
						<th style="width: 100px;text-align: right;">最后修改时间</th>
	...
	<%-- 窗口 modal:true为模式窗口 
		...
		<div id="win" class="easyui-window" title="My Window"
			style="width: 315px; height: 300px" data-options="modal:true,collapsible:false,maximizable:false,minimizable:false,draggable:false,resizable:false"
			>
			<%-- 布局 --%>
			<div id="p" class="easyui-panel" 
						style="background: #fafafa;">
				<table class="table table-hover table-condensed" id="friend">
					<tr>
						<td>编号</td>
						<td>昵称</td>
					</tr>
