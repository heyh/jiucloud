...
<script type="text/javascript">
    ...
    function add(row) {
        if (document.getElementById("norecord") != undefined) {
            document.getElementById("addStockTabBody").removeChild(document.getElementById("norecord"));
        }
        var trObj = document.createElement("tr");
        trObj.id = "tr_" + row.id;
        trObj.innerHTML =
            "<td style='text-align:center;'>" + document.getElementById("addStockTable").rows.length + "</td>" +
            "<td>" + row.mc + "</td>" +
        ...
        document.getElementById("addStockTabBody").appendChild(trObj);
    }

    function del(_id) {
        document.getElementById("addStockTabBody").removeChild(document.getElementById("tr_" + _id));
        for (var i = 1; i < document.getElementById("addStockTable").rows.length; i++) {
            document.getElementById("addStockTable").rows[i].cells[0].innerHTML = i;
        }
        $('#' + "gridtree_row_" + _id).removeAttr("checked");

        if (document.getElementById("addStockTable").rows.length == 1) {
            var trObj = document.createElement("tr");
            trObj.id = "norecord";
    ...
    function geneAddStock() {
        var tableObj = document.getElementById("addStockTable");
        var tableInfo = [];
        ...
        <div class="layui-col-xs6">
            <form class="form-horizontal" name="form" id="form" method="post" enctype="multipart/form-data" role="form">
                ...
                <blockquote class="layui-elem-quote" style="text-align: center;height: 25px">
                    <a style="font-size:16px;">材料入库信息</a>
                </blockquote>
                <table class="table_style table table-striped table-bordered table-hover table-condensed"
                    ...
                    <tbody id="addStockTabBody">
                    <tr id="norecord">
                        <td colspan='100' style='text-align:center;'>温馨提示:勾选左侧材料库材料，进行入库操作!</td>
                    </tr>
                    </tbody>
                </table>
