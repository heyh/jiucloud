...
<script type="text/javascript">
	...
	$(function() {
		...
		dataGrid = $('#dataGrid').datagrid({
			url : '${pageContext.request.contextPath}/messageController/datarecGrid',
			fit : true,
			fitColumns : true,
			border : false,
			pagination : true,
	...
	function deleteFun(id) {
		...
		parent.$.messager.confirm('询问', '您是否要删除当前数据？', function(b) {
			if (b) {
	        	...
	        	$.ajax({
						type: "post",
						url: '${pageContext.request.contextPath}/messageController/deleteInRec',
						 data: {id:id},
						  dataType: "json",
		...
		if (rows.length > 0) {
			parent.$.messager.confirm('确认', '您是否要删除当前选中的数据？', function(r) {
				if (r) {
					...
					$.getJSON('${pageContext.request.contextPath}/messageController/batchDeleteRec?time='+getTimestamp, {
						ids : ids.join(',')
					}, function(result) {
						if (result.success) {
	...
	function editFun(id) {
		...
		parent.$.modalDialog({
			...
			width : 720,
			height : 500,
			href : '${pageContext.request.contextPath}/messageController/recread?msgid=' + id
