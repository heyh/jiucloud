...
<script type="text/javascript">
	...
	$(function() {
		dataGrid = $('#dataGrid')
				.datagrid(
						{
							url : '${pageContext.request.contextPath}/fieldDataController/dataGrid?source=material',
							fit : true,
							fitColumns : false,
							border : false,
							pagination : true,
...
//									},
...

									{
										...
										formatter : function(value, row, index) {
                                            ...
                                            var userId = <%= userId%>;
                                            var strHasOutRight = <%=hasOutRight%>;
                                            if(((compareDate(getCurrentDate(), row.creatTime.substring(0, 10)) == 0 || '9' == row.needApproved) && userId == row.uid  && '0' == row.isLock && '2' != row.needApproved) || (row.itemCode.substring(0, 3) == '800' && <%= hasReadEditRight %> )) {
                                                    str += $
                                                            .formatString(
...
//		str += '&uname=' + $('#uname').val();
		...
		var url = "${pageContext.request.contextPath}/fieldDataController/securi_execl?a=1&source=material"
				+ str;
		window.open(url);
	}
