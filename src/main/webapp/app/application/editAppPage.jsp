...
<script type="text/javascript">
	$(function() {
		...
		$("#img").show().attr("src", "${pageContext.request.contextPath }/upload/icon/"+path);//显示图片
		 ...
		 //初始下拉框选中
		var isok=$('#isok').val();
		   $("#rep  option[value="+isok+"]").attr("selected",true);
		//获得文件大小  
...
<div class="easyui-layout" data-options="fit:true,border:false">
	<div data-options="region:'center',border:false" title="" style="overflow: hidden;">
		<input type="hidden" id="isok" value="${app.isRep}"/>
		<form id="form" method="post" enctype="multipart/form-data">
			<input type="hidden" name="rTime" value="${app.repTime}"/>
			<input type="hidden" name="id" value="${app.id}"/>
			<table class="table table-hover table-condensed">
				<tr>
					<td style="width: 70px;">应用名称</td>
					<td>
						<input name="appName" type="text" placeholder="请输入应用名称" class="easyui-validatebox span2" data-options="required:true" value="${app.appName}">
				...
				<tr>
					...
					<td>
						<input type="text" name="icon" id="icon" value="${app.icon}" readonly="readonly">
				...
				<tr>
					...
					<td>
						<input type="text" name="upPath" id="upPath" value="${app.upPath}" readonly="readonly">
				...
				<tr>
					...
					<td>
						<input name="appSize" id="appSize" type="text" placeholder="文件大小" class="easyui-validatebox span2" readonly="readonly" value="${app.appSize}">
					</td>
				</tr>
				<tr>
					<td style="width: 70px;">版本号</td>
					<td>
						<input name="versionNum" type="text" placeholder="版本号" class="easyui-validatebox span2" data-options="required:true" value="${app.versionNum}">
					</td>
				</tr>
				<tr>
					<td style="width: 70px;">包路径</td>
					<td>
						<input name="sketch" type="text" placeholder="请输入包路径" class="easyui-validatebox span2" data-options="required:true" value="${app.sketch}">
					</td>
				</tr>
				<tr>
					<td style="width: 70px;">内容</td>
					<td>
						<textarea name="content" placeholder="请输入内容" style="width:300px;height:130px;" data-options="required:true" class="easyui-validatebox span2">${app.content}</textarea>
					</td>
				</tr>
			</table>
		</form>
