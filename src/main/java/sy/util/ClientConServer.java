...
public class ClientConServer {
	...
	public ClientConServer() {
		...
		// 初始化connection 对象
		connection = getConnection(bundle.getString("XMPP_serverIp"),
				Integer.parseInt(bundle.getString("XMPP_serverPort")));
	 ...
	 * @return
	 */
	private XMPPConnection getConnection(String _serverIp, int _serverPort,String admin,String pwd){
	...
	/**
	 * 构造方法，用于初始化connection,此方法主要用在注册时的初始化 x
	 * 
	 * @param _serverIp
	 * @param _serverPort
	...
	public boolean login(String _username, String _password) {
		...
		try {
		 	System.out.println(connection+"   =================== u:"+_username+"   p:"+_password);
			connection.connect();
	...
	/*
	 * public android.graphics.drawable.Drawable getUserImage(String user) { if
	 * (getConnection() == null) return null; ByteArrayInputStream bais = null;
	 * try { VCard vcard = new VCard(); // 加入这句代码，解决No VCard for
	 * ProviderManager.getInstance().addIQProvider("vCard", "vcard-temp", new
	 * org.jivesoftware.smackx.provider.VCardProvider()); if (user == "" || user
	 * == null || user.trim().length() <= 0) { return null; }
	 * vcard.load(getConnection(), user + "@" +
	 * getConnection().getServiceName());
	 * 
	 * if (vcard == null || vcard.getAvatar() == null) return null; bais = new
	 * ByteArrayInputStream(vcard.getAvatar()); } catch (Exception e) {
	 * e.printStackTrace(); return null; } return
	 * FormatTools.getInstance().InputStream2Drawable(bais); }
	 */
	...
	public boolean addGroup(String groupName) {
		if (connection == null)
			...
			ResourceBundle bundle = ResourceBundle.getBundle("config");
			String admin = bundle.getString("XMPP_ADMIN");
			String pwd = bundle.getString("XMPP_PWD");
			try{
				connection.connect();
				connection.login(admin, pwd);
				}catch(Exception ex){
					ex.printStackTrace();
				}
			Roster roster = connection.getRoster();
			  ...
			  // String[] groups = new String[] { groupName };
			   //roster.createEntry(admin+"@wjl", "william", groups);
	...
	public List<HashMap<String, String>> searchUsers(String userName) {
		...
		try {
			...
			org.jivesoftware.smackx.ReportedData data = usm.getSearchResults(
					answerForm, "search" + connection.getServiceName());
	...
	public String regAccount(String _uname, String _pwd,
			String date) {
		// 这里有点疑惑，这里使用AccountManager中的createAccount方法和使用Registration的区别是什么
		try {
			// accountmanage.createAccount(_uname, _pwd, attribute);
			Registration reg = new Registration();
			reg.setType(IQ.Type.SET);
			reg.setTo(ClientConServer.connection.getServiceName());
		 
			reg.setUsername(_uname);
			reg.setPassword(_pwd);
			reg.addAttribute("date", date);
	...
	/**
	 * * 加入会议室 * * @param user * 昵称 * @param password * 会议室密码 * @param roomsName
	 * 会议室名
	 */
	...
	public int IsUserOnLine(String user) {
		String SERVER_NAME = "wjl";
		String SERVER_HOST = "my.openfire.com:9090";
	...
	public void listeningConnectToServer() {
		 ...
		 * super.handleMessage(msg); boolean isConnected = (Boolean) msg.obj;
		 * isConnectOK = isConnected; if(!isConnectOK){
		 * Toast.makeText(ChatMainActivity.this, "网络断开!",
		...
		Runnable connectToServeListerRunable = new Runnable() {
			...
			public void run() {
				// clintconnserver = new
				// ClientConServer(cononcetToServerHandler);
	...
	/**
	 * 原因 the smack.providers file, usually in /META-INF folder in normal
	 * versions of smack, can't be loaded in Android because its jar packaging.
	 * So all the providers must be initialized by hand <br/>
	 * 参考网址：http://stackoverflow.com/questions/5910219/problem-using-usersearch-
	 * in-xmpp-with-asmack
	 * 
	 * @param pm
	 */
